...
import formatWeatherData from '@/lib/formatWeatherData';
...
function ContextProvider({ children }: ContextWrapperProps) {
  ...
  useEffect(() => {
    ...
    const tid = toast.loading(
      "Fetching your location's weather information...",
    );
    fetchInformation(position)
      .then((info) => {
        if (info && info.length) {
          const data = info[0];
          setInformation({
      ...
      .catch((err) => {
        console.error(err);
        toast.error(
          "Failed to get your location's weather information. Reload and try again.",
        );
      });
  ...
  useEffect(() => {
    ...
    fetchForecast(position)
      .then((info) => {
        if (info) {
          setForecast(formatWeatherData(position, info));
          toast.dismiss(tid);
        }
      })
      .catch((err) => {
